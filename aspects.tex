...
We can characterize the solutions using the Pareto-front. Therfore, we say that a point $x = \langle \scp,\rndp \rangle$ is dominated by $x' = \langle \scp', \rndp' \rangle$, in symbols $x \prec x'$, if $x'$ is strictly larger in at least one dimension and not smaller in the other, that is, 
\begin{equation}
...
The Pareto-front $\pareto{\solutions}$ of $\solutions$ is given as \[ \{ x \in \solutions \mid \forall x' \in \solutions, x \not\prec x'  \}. \]
