...
\mypara{Results} 
First and foremost, we succeed to synthesize controllers in the mentioned setup.
The controller randomizes its behavior while meeting the specification, which is not surprising as the algorithm yields a correct-by-construction policy.
\footnote{Please consult the supplementary material where we include a video with various traces generated}.

We consider the practical run time of our algorithm. 
...
\sj{The same fig is referenced twice}


As demonstrated in Fig.~\ref{fig:exp_times},
the use of BDDs dramatically decreases the size
of the game graph with negligible overhead. \footnotemark 
