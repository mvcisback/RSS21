...
\begin{mdframed}[nobreak=true]
 ...
 Second, RCI fails to enforce randomization if there exists *any* path
...
the maximum number of paths guarantee-able by
$\pOne$~\cite{DBLP:conf/cav/FremontS18}. We remark that because (1)
causal entropy reduces to non-causal entropy in deterministic
dynamics and (2) uniform distributions maximize entropy, our proposed
entropy matching family exactly agrees with existing RCI algorithms on
...
\begin{proposition}\label{prop:conservative}
  ...
  \[f : (\ppthreshold, \sg) \mapsto \randomness,\] such that, for any
