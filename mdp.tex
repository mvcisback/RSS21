...
$\rat \in [0, \infty)$, we define a policy $\sched_\rat$ -- using $s = \last{\path}$ -- as follows:
 \begin{align}
...
\rndp_{\sched_\rat}$. 
We remark that for $\lambda \rightarrow \infty$, this variant of the Bellman equations coincides with the standard Bellman equations~\cite{DBLP:books/wi/Puterman94}.
...
In particular, we construct $\pareto{} = \{ x_\rat \mid \rat \in \{ \rat_1, \rat_2, \hdots \} \}$ until $\pareto{}$ contains a witness to either realizability or unrealizability. 
In the remainder of this section, we improve upon randomly selecting values for $\rat$.

\begin{remark}
Given a witness(pair) to realizability, it is easy to extract the corresponding improvisor. Let $x_1,x_2$ be a witness-pair to realizability,  induced by $\sched_{\rat_1}$ and $\sched_{\rat_2}$ such that $\langle\scthreshold, \randomness\rangle\preceq q\cdot x_1 + \bar{q}\cdot x_2$, then the policy described by  	
