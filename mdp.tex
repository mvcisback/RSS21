...
on SGs. To start, recall that an MDP is a stochastic game with no action choices for the environment, i.e., the environment is purely stochastic. The
key idea is to rephrase the tradeoff between randomization and
...
Thus, this
family spans the Pareto-Front, $\pareto{\solutions}$ and by varying $\rat$ we can explore the Pareto-front. First observe the following easily verified proposition.
\begin{proposition}
