...
\mypara{Environment Policies} We begin with two observations
...
this specific $\pTwo$-policy, $\sched^\rat_\pTwo$, optimizing in the
...
of the subgraphs. By induction, (with the MDP base case), we obtain
continuity and strict monotonicity.

\mypara{Memoizing Pareto Fronts}
...
\todo[inline]{Visualize this in terms of intersections and different
  directions. Attach to game graph and annotate up arrow for dynamic programming and down error for MDP subroutine} Thus, by traversing the graph from the sinks to the
...
%   all subgraphs accessed by its actions. Thus $\sched_\pTwo^\rat$
%   remains unchanged.
