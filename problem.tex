...
SGs capture a variety of models.  For example, if $\EnAct(s)$ is a
singleton set for all uncontrolled states, then $\sg$ is a
\emph{Markov decision process} (MDP).  If $\EnAct(s)$ is a singleton
set for all states, then $\sg$ is a \emph{Markov chain}. If
...
\begin{enumerate}
  ...
  \item Causal and non-causal conditioning can be mixed, e.g,
    ...
    \end{equation}
\end{enumerate}
Intuitively, the distinction is that causal entropy does not
condition on variables that have not been revealed. It has been observed
that this makes causal entropy particularly well suited for measuring
predictability in \emph{sequential} decision making problems, where
