...
As in \cite{DBLP:conf/cav/Vazquez-Chanlatte20}, we may represent this computation tree (typically) concisely using binary decision diagrams.
 ...
 The probability $\Pr^\mc(\eventually{h} T)$ denotes the probability to reach a state in $T$ with $h$ steps in the induced Markov chain, i.e., $\Pr^\mc(\eventually{h} T)$.
For any SG $\sg$ and scheduler $\sched$, let $\Pr^\sg_\sched(\eventually{h} T)= \Pr^{\induced{\sg}{\sched}}(\eventually{h} T)$.  

\begin{mdframed}[backgroundcolor=white!5]
\textbf{The ERCI Pareto Front Problem}:
Given an acyclic SG $\sg$ with target states $\mathbf{1}$ and sink state $\mathbf{0}$, find the set $\solutions \subseteq \mathbb{R}^2$ of tuples $\langle \scthreshold, \randomness \rangle$, such that there exists a $\pOne$-policy  such that for every policy $\pTwo$-policy with $\sched = \langle \pOneSched, \pTwoSched \rangle$ it holds that 
\begin{compactenum}
	\item (\emph{soft constraint)} $\Pr^\sg_{\sched}(\eventually{h} G) \geq \scthreshold$
	\item (\emph{randomness constraint}) $H_\sg(\sigma) \geq \randomness$
\end{compactenum}
\end{mdframed}
