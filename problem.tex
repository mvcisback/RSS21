...
In this subsection, we discuss two modifications of the problem statement.
First, we are interested in understanding the combinations of $\scthreshold$ and $\randomness$ that allow to find a policy at all. 
Second, to ease the technical construction, without loss of generality, we make the following assumptions\footnote{We argue that these assumptions are indeed w.l.o.g.\ in Sec.~\ref{sec:assumptions}}: 
...
Naturally, we are still interested in finding the corresponding policies for a any point $\langle \scthreshold, \randomness \rangle$, and this is a side product of our algorithm. 
