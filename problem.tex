...
entropy (and thus non-negative). Intuitively, the distinction is that causal entropy does not
...
Given a SG $\sg$, $\tau$-bounded path properties $\psi$ and $\varphi$, and thresholds $\scthreshold \in [0,1]$ and $\randomness \in [0,\infty)$, find a $\pOne$-policy $\pOneSched$  such that for every $\pTwo$-policy $\pTwoSched$,
\begin{compactenum}
...
\footnotetext{Formally, the state space is quotiented by distinguishablity under $\varphi$ and $\psi$.}

Next, in order to ensure the hard constraint, $\psi$, we
...
lexicographic pass over $\sg$ from the leaves to the root.  We remove
...
$\sink$, based on membership in $\varphi$, i.e.,
\begin{equation}
  \begin{split}
    \last{\path} = \target ~\implies~ \path \in \varphi\\
    \last{\path} = \sink ~\implies~ \path \notin \varphi
  \end{split}.
\end{equation}
This yields the following (core) ERCI problem.
\begin{mdframed}
\textbf{The Entropic Control Improvisation (ERCI) Problem}:
